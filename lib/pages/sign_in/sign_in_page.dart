...
class _SignInPageState extends State<SignInPage> {
  // Флаг теперь живет внутри состояния экрана и сбросится при его уничтожении
  bool _nextScreenLoading = false;

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    return ViewModelBuilder(
      create: () => SignInViewModel(Matrix.of(context), signUp: widget.signUp),
      builder: (context, viewModel, _) {
        ...
        if (state.loginLoading.connectionState != ConnectionState.waiting &&
          ...
          // Меняем состояние внутри фреймворка
          _nextScreenLoading = true;

          Future.microtask(() {
            if (selectedHomeserver != null &&
                context.mounted &&
                state.loginLoading.connectionState != ConnectionState.waiting) {
              Logs().i('future microtask ${selectedHomeserver.name!}');
...
//         //Logs().i(selectedHomserver.name!);

//         if (state.loginLoading.connectionState == ConnectionState.waiting || !context.mounted || selectedHomserver == null) {
//           return Scaffold(
//             backgroundColor: theme.colorScheme.surface,
//             body: const Center(
//               child: CircularProgressIndicator.adaptive(), // Адаптивное колесико (iOS/Android)
//             ),
//           );
//         }
...
//         if (state.loginLoading.connectionState != ConnectionState.waiting && !_nextScreenLoadind) {
//           _nextScreenLoadind = true;
        
//           Future.microtask(() {
            
//             // 3. Если сервер определился, запускаем флоу
//             if (selectedHomserver != null && context.mounted && state.loginLoading.connectionState != ConnectionState.waiting) {
//               Logs().i('future microtask ${selectedHomserver.name!}');
//               connectToHomeserverFlow(
//                 selectedHomserver,
//                 context,
//                 viewModel.setLoginLoading,
